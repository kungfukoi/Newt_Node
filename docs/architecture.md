...
A built Director package can control supported Video Model settings, drive an Image Model prompt, and provide scene/reference context to Storyboard. `src/filmDirectorModelRouting.js` owns the shared Director input contract, prompt composition, and visual-reference merge used by Image and Video routes. The Image route sends active Director location/prop references as image prompts and active character references as character inputs while preserving direct downstream inputs. Storyboard validates cut order, required continuous-shot keyframes, frame numbering, and nonempty prompts before replacing visible work. Failed planning leaves the existing board intact. Visual QC distinguishes a reviewed result from `unreviewed` when the review service is unavailable.
