...
- Atlas HTTP 402 is a confirmed billing rejection (insufficient balance or exhausted allowance), displayed as a failed run with billing guidance rather than unknown acceptance. It is never automatically resubmitted. Older records already marked uncertain retain their original state.
...
The node progress display includes the provider's latest state and a job-specific heartbeat. Krea Seedance 2.5 is marked delayed after 15 minutes of provider tracking and possibly stalled after 30 minutes. These are warnings, not terminal states: only a provider response of `completed`, `failed`, or `cancelled` ends the run. NewtNode continues polling the original request ID so a delayed result is not replaced by a second potentially billable submission.
