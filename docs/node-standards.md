...
- Nano Banana Pro routing is explicit: Settings or `NANO_BANANA_PROVIDER` may select `google` or `fal`. Google routing requires `GOOGLE_API_KEY`; Fal routing uses the configured Fal Nano Banana Pro endpoint.
