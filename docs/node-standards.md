...
- Film Director approach values are stable persisted identifiers: `cinematic`, `vintage`, `animation`, `stop-motion`, `commercial`, `music-video`, and `montage`. Approach-specific style, camera, scene rules, audio behavior, and revisions must flow through `src/filmDirectorApproaches.js`; do not implement approach labels as prompt-only UI decoration.
...
- Film Director tracks dependency signatures for Setup, Style Direction, Camera Direction, Scene Overview, and Shot List. Input changes mark only affected stages stale, retain their visible drafts and graph connections, and never start paid regeneration. Lock accepts the current draft; Regenerate is an explicit user action.
- Film Director may select `Use Connected Model` or an explicit supported Video Model. An explicit selection controls the downstream model, duration, resolution, aspect ratio, and audio mode; the Video Model must show the effective values. Unsupported models keep their existing behavior.
- Film Director audio modes are `Production Sound`, `Full Audio`, and `Silent`. Production Sound preserves dialogue and restrained location sound while forbidding score; Full Audio permits native synchronized audio; Silent disables native and connected audio. Every provider path must honor the same effective audio decision.
