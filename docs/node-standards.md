...
- Generation elapsed time must remain monotonic for the full run, including runs longer than one minute. The latest terminal elapsed time remains visible on its node until that node submits another run; completion freezes the elapsed time rather than continuing to count.
