...
- **Director and Storyboard**: Build structured shot direction with Cinematic, Vintage, Animation, Stop-Motion, Commercial, Photography, Music Video, and Montage approaches. `Still` duration locks Director to one image shot, and a built Director package can feed its prompt plus active references directly into an Image Model as well as supported Video Models and Storyboard. Image Models show the actual expanded reference count, refuse silent truncation, and let directly connected Style and Camera nodes override conflicting Director direction. Optional music and reference-video inputs can drive timing, camera, performance, extension, or visual reference behavior. Storyboard planning validates every cut and required keyframe before replacing existing work, and visual QC reports unavailable review honestly instead of treating it as a pass. See [Creative Workflows](docs/creative-workflows.md).
...
- **Current 3D models**: Generate GLB assets with Hunyuan 3D 3.1 Pro through Fal or Krea, or Rodin 2.5 through Fal, from the 3D node's model menu.
...
- **Video handling**: Video thumbnails and result panes keep native scrub controls; use their drag grip or Ctrl-drag the video to create a media node or add the output directly to a Timeline bin.
