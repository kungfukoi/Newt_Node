...
import { allowFileDrop, displayMediaUrl, fullResolutionImageProps, outputItemFromDataTransfer, previewImageUrl } from "../mediaAssets.js";
    ...
    return (
      <div className="style-collage transfer-output-preview locked">
        <div className="style-collage-cell">
          <img {...fullResolutionImageProps(outputUrl, outputLabel)} src={displayMediaUrl(previewImageUrl(outputUrl))} alt={outputLabel} onError={useNewtNodeImageFallback} />
    ...
    <div className={`style-collage count-${images.length} ${locked ? "locked" : ""}`} onDragOver={allowFileDrop} onDrop={handleDrop}>
      {images.map((image) => (
        <div className="style-collage-cell" key={image.id}>
          <img {...fullResolutionImageProps(image)} src={displayMediaUrl(previewImageUrl(image))} alt={image.fileName || "Mood board reference"} onError={useNewtNodeImageFallback} />
          {!locked && (
